...
# Config
# ---------------------------------------------------------------------------
TOKEN_TTL_SECONDS = int(os.environ.get("TOKEN_TTL_SECONDS", "840"))  # 14 min
...
# Solver integration
SOLVER_ENABLED = os.environ.get("SOLVER_ENABLED", "0") == "1"
SOLUTION_SAVE_ENDPOINT = os.environ.get("SOLUTION_ENDPOINT", "http://localhost:9999/save-solution")

# ---------------------------------------------------------------------------
# State (thread-safe)
# ---------------------------------------------------------------------------
_lock = threading.RLock()
_pool: Deque[Tuple[str, float]] = deque()  # (token, created_ts)
_seen: set[str] = set()
_stats = {
    "total_received": 0,
    "total_served": 0,
...
_rate_window: Deque[float] = deque(maxlen=300)
...
def _purge_expired() -> int:
    """Remove tokens older than TTL. Must hold _lock."""
    now = time.time()
    removed = 0
...
def _make_token() -> str:
    """Generate CN31-style token."""
    return secrets.token_hex(32)


def _add_token(token: Optional[str] = None) -> Dict[str, Any]:
    """Add token to pool."""
    with _lock:
        _purge_expired()
...
def _get_stats() -> Dict[str, Any]:
    with _lock:
        ...
        return {
            ...
            "solver_tasks": _stats["solver_tasks"],
            "solver_solved": _stats["solver_solved"],
        }
...
def save_solution():
    """Solver posts solved tokens here."""
    body = request.get_json(silent=True) or {}
...
def dashboard_assets(filename: str):
    return send_from_directory(os.path.join(BASE_DIR, "dashboard-assets"), filename)
...
def _ensure_started() -> None:
    ...
    with _start_lock:
        ...
        if SOLVER_ENABLED:
            print(f"[CN31] Solver integration enabled", flush=True)
...
def main() -> None:
    _ensure_started()
    print(f"[CN31] Listening on {HOST}:{PORT}", flush=True)
