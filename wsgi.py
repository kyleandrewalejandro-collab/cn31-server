...
# ===========================
# Dun163 Proxy (for real token solving)
# ===========================
...
def _get_dun163_domain():
    """Round-robin domain selection."""
    domains = os.environ.get("DUN163_DOMAINS", "").split(",")
    if domains and domains[0]:
        return domains[0].strip()
    return DUN163_DOMAINS[0]

@app.route("/dun163/getconf", methods=["POST"])
def dun163_getconf():
    """Proxy: Get Dun163 config."""
    try:
        data = request.get_json(silent=True) or {}
        domain = _get_dun163_domain()
        url = f"{domain}/api/v2/getconf"
        resp = req_module.post(url, json=data, timeout=10)
        return resp.text, resp.status_code, {"Content-Type": "application/json"}
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@app.route("/dun163/get", methods=["POST"])
def dun163_get():
    """Proxy: Get captcha challenge."""
    try:
        data = request.get_json(silent=True) or {}
        domain = _get_dun163_domain()
        url = f"{domain}/api/v2/get"
        resp = req_module.post(url, json=data, timeout=10)
        return resp.text, resp.status_code, {"Content-Type": "application/json"}
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@app.route("/dun163/check", methods=["POST"])
def dun163_check():
    """Proxy: Check solution and get validated token."""
    try:
        data = request.get_json(silent=True) or {}
        domain = _get_dun163_domain()
        url = f"{domain}/api/v2/check"
        resp = req_module.post(url, json=data, timeout=10)
        return resp.text, resp.status_code, {"Content-Type": "application/json"}
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def monitor():
    try:
        ...
        return jsonify({"message": "Monitoring dashboard"}), 200
